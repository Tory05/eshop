...
class CartView extends View{
    ...
    createView($data) {
        ...
        if($data['cart'].length === 0){
            $('.cart-items').html('').html('Kosár üres!');
        }
        ...
        $.each($data['cart'], function($row, $product){
            ...
            let $markup = `<tr>
                ...
                <td class="cart-product-price">${$product["price"]} Ft</td>
                <td class="cart-total-price">${$totalPerProductPrice} Ft</td>
        ...
        $('.total-order-price').html($roundedTotal+" Ft");
